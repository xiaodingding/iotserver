...
class Command(BaseCommand):
    help = ('Connect with client as subscriber, for test proposed')
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--id', action='store',
                            type=str, default=None, dest='id',
                            help=('id from DB object')
                            )
        ...
        parser.add_argument('--client_id', action='store',
                            ...
                            help=('client_id for broken')
                            )

    def handle(self, *args, **options):
        if not options['topic']:
            raise CommandError(('Topic requiered and must be only one'))
        apply_filter = {}
        ...
        try:
            ...
            raise CommandError(('Client not exist'))
