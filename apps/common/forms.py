...
class BasicSettingForm(BaseForm):
    SITE_URL = forms.URLField(
        ...
        help_text="http://yuechifan.net:8080"
    )
...
class EmailSettingForm(BaseForm):
    EMAIL_HOST = forms.CharField(
        max_length=1024, label=_("SMTP host"), initial='smtp.yuechifan.net'
    )
    EMAIL_PORT = forms.CharField(max_length=5, label=_("SMTP port"), initial=25)
    EMAIL_HOST_USER = forms.CharField(
        max_length=128, label=_("SMTP user"), initial='noreply@yuechifan.net'
    )
...
class LDAPSettingForm(BaseForm):
    ...
    AUTH_LDAP_BIND_DN = forms.CharField(
        label=_("Bind DN"), initial='cn=admin,dc=yuechifan,dc=org'
    )
    ...
    AUTH_LDAP_SEARCH_OU = forms.CharField(
        label=_("User OU"), initial='ou=tech,dc=yuechifan,dc=org'
    )
...
class NeutralSetting(BaseForm):
    SERVER_SITE_NAME = forms.CharField(
        label=_("site name"), initial='IOT Server'
    )
    SERVER_SITE_LOGE = forms.CharField(
        label=_("site logo"), initial='/static/img/logo-text.png'
    )
    SERVER_SITE_COPYRIGHT = forms.CharField(
        label=_("site copyright"), initial='DFine 工作室'
    )
    SERVER_SITE_URL = forms.CharField(
        label=_("site url"), initial='yuechifan.net'
    )
    SERVER_SITE_INTRODUCE =  forms.CharField(
        label=_("site introduce"), initial='请简要编写平台介绍'
    )